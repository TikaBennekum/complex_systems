...
def analyze_num_streams_experiment(data_file, output_file=None, slopes = np.linspace(0.05,1,10), erosions=[0.025,0.05,0.1,0.2,0.4],):
    """
    Analyzes the data generated by the number of streams experiment with varying combinations of flows and slopes
    ...
    mean_num_streams_graph(num_streams, output_file, slopes, erosions)
    plt.imshow(num_streams[:,2])
    plt.colorbar()
    plt.show()
...
def mean_num_streams_graph(data, output_file = None, slopes=np.linspace(0.01,1,10), erosions=[0.025,0.05,0.1,0.2,0.4]):
    """
    plots the mean number of streams in a cross section dependent on slope for different water flow amounts 
...
if __name__ == "__main__":
    ...
    # num_streams_experiment_flow('data/grids_data1', num_steps=10000, slopes = slopes,flows=[0.25, 0.5, 1, 2, 4], steps_per_gen=1000)
    ...
    analyze_num_streams_experiment('data/grids_data_erosion.npy', 'slope_phase_trans_erosion.png', slopes=slopes, erosions=erosions)
