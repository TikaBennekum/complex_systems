...
def exp_incr_flow_decr_eros():
    ...
    labels = [
        f"k: {k_val:.2f}, flow_rate: {flow_rate_val}"
        for k_val, flow_rate_val in zip(k, flow_rate)
    ]

    # Plot data
    plt.figure(figsize=(10, 6))
    plt.scatter(
        range(len(flow_rate)), total_erosion, marker="o"
    )  # Use indices for x-values
    plt.xticks(
        ticks=range(len(flow_rate)), labels=labels, rotation=45, ha="right"
    )  # Add combined labels
    plt.xlabel("Slope", fontsize=16)
    plt.ylabel("Total Erosion", fontsize=16)
    plt.title("Total erosion for linearly increasing slopes", fontsize=18)
...
def exp_incr_slope_incr_eros():
    ...
    # Create combined labels for x-axis
    labels = [
        f"k: {k_val:.2f}, slope: {np.round(slope_val, 3)}"
        for k_val, slope_val in zip(k, slope)
    ]

    # Plot data
    plt.figure(figsize=(10, 6))
    plt.scatter(
        range(len(slope)), total_erosion, marker="o"
    )  # Use indices for x-values
    plt.xticks(
        ticks=range(len(slope)), labels=labels, rotation=45, ha="right"
    )  # Add combined labels
