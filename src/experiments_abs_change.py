...
def change_every_n_steps(initial_state, steps, steps_per_gen, metric, skip_initial_gens = 0, params=default_params, update_progress = False):
    """
    ...
    for gen in range(num_gens):
        ...
        if(gen >= skip_initial_gens):
            diffs[gen] = metric(last_state[:,:,WATER_HEIGHT], gen_grids[-1,:,:,WATER_HEIGHT])
...
def small_grid_change_experiment(output_file, slope = 0.1, flow = 1, width=21, height=100, num_steps = 10000, steps_per_gen=100, params=default_params, metric=mse):
    np.random.seed(42)
