...
def entropies(grid, threshold = 1e-9, skip_rows = 1):
    """
    ...
    # entropies = np.zeros(height)
    # for row in range(skip_rows, height -skip_rows):
        
    #     row_vals = grid[row,:,WATER_HEIGHT] 
    #     row_vals = row_vals / np.sum(row_vals)
    #     row_vals *= row_vals > 1e-9
    #     entropy =  row_vals * np.log2(row_vals)
    #     entropy[np.isnan(entropy)] = 0
    #     entropies[row] = np.sum(entropy)
    # grid1 = grid[:,:,WATER_HEIGHT]    
    # print(grid1.shape) 
    grid1 = np.maximum(1e-9, grid)
    # grid1 /= np.sum(grid1, axis=1, keepdims=True)
    return entropy(grid1, base=2)
...
def num_streams_experiment_flow(output_file, slopes = np.linspace(0.01,10,20), flows=[0.25, 0.5, 1, 2, 4], width=101, height=1000, num_steps = 10000, steps_per_gen=100, params=default_params):
    """
...
def analyze_num_streams_experiment_flow(data_file, output_file=None, slopes = np.linspace(0.05,1,10), flows=[0.25, 0.5, 1, 2, 4]):
    """
...
def mean_num_streams_graph_flow(data, output_file = None, slopes=np.linspace(0.01,1,10), flows=[0.25, 0.5, 1, 2, 4]):
    """
...
def num_streams_experiment_erosion(output_file, slopes = np.linspace(0.01,10,20), erosions=[0.025,0.05,0.1,0.2,0.4], width=101, height=1000, num_steps = 10000, steps_per_gen=100, params=default_params):
    """
    ...
    Returns:
        grids: array of shape [num_slopes x num_flows x num_steps //steps_per_gen x height x width x NUM_CELL_FLOATS]  saved in output_file     
    """
    data = np.zeros([len(slopes), len(erosions), num_steps // steps_per_gen, height, width, NUM_CELL_FLOATS])
    for i, slope in enumerate(slopes):
        for j, erosion_k in enumerate(erosions):
            params['EROSION_K'] = erosion_k
            grids =  run_single_experiment(height=height, width=width, mean_slope=slope, num_steps=num_steps, steps_per_gen=steps_per_gen, flow=1, params=params)
            data[i, j] = grids  
    if output_file is not None:      
        np.save(output_file, data)
               
               
def analyze_num_streams_experiment_erosion(data_file, output_file=None, slopes = np.linspace(0.05,1,10), erosions=[0.025,0.05,0.1,0.2,0.4],):
    """
    Analyzes the data generated by the number of streams experiment with varying combinations of flows and slopes
...
def mean_num_streams_graph_erosion(data, output_file = None, slopes=np.linspace(0.01,1,10), erosions=[0.025,0.05,0.1,0.2,0.4]):
    """
    plots the mean number of streams in a cross section dependent on slope for different water flow amounts 
...
def analyze_num_streams_experiment_flow_entropy(data_file, output_file=None, slopes = np.linspace(0.05,1,10), flows=[0.25, 0.5, 1, 2, 4]):
    """
    Analyzes the data generated by the number of streams experiment with varying combinations of flows and slopes
    ...
    print(data.shape)
    width = data.shape[4]
    mean_entropies = np.zeros([len(slopes), len(flows)])
    for i, slope in enumerate(slopes):
        for j, flow in enumerate(flows):
            mean_entropies[i,j] = np.mean(entropies(data[i,j,-1]))
...
def analyze_num_streams_experiment_erosion_entropy(data_file, output_file=None, slopes = np.linspace(0.05,1,10), erosions=[0.025,0.05,0.1,0.2,0.4],):
    """
    Analyzes the data generated by the number of streams experiment with varying combinations of flows and slopes
...
def mean_num_streams_graph_erosion_entropy(data, output_file = None, slopes=np.linspace(0.01,1,10), erosions=[0.025,0.05,0.1,0.2,0.4]):
    ...
    # assert data.shape[1] == len(flows)
    for i,erosion in enumerate(erosions):
        plt.plot(slopes, data[:,i], linestyle = '-', marker = 'o', label=(erosion))
    # plt.yscale('log')
    # plt.xscale('log')
    plt.ylabel('H')
    plt.xlabel('mean slope')
    plt.title('Mean Cross Section Entropy for different Erosion Rates')
    plt.legend(title='Erosion_rate')
    if output_file is not None:
        plt.savefig(output_file, dpi=600)
    plt.show()
...
if __name__ == "__main__":
    ...
    # analyze_num_streams_experiment_flow('data/grids_data1.npy', 'plots/slope_phase_trans_flow.png', slopes=slopes, flows=flows)
    analyze_num_streams_experiment_flow_entropy('data/grids_data1.npy', 'plots/slope_phase_trans_flow_entropy.png', slopes=slopes, flows=flows)
