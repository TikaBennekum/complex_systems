...
def mean_num_streams_graph_flow(
    ...
    for i, flow in enumerate(flows):
        plt.plot(
            slopes, mean_num_streams[:, i], linestyle="-", marker="o", label=(flow)
        )
...
def mean_num_streams_graph_erosion(
    ...
    for i, erosion in enumerate(erosions):
        plt.plot(
            slopes, mean_num_streams[:, i], linestyle="-", marker="o", label=(erosion)
        )
    plt.ylabel("number of streams")
    plt.xlabel("mean slope")
...
def analyze_num_streams_experiment_flow_entropy(
    data_file,
    output_file=None,
    slopes=np.linspace(0.05, 1, 10),
    flows=[0.25, 0.5, 1, 2, 4],
...
def mean_num_streams_graph_flow_entropy(
    data, output_file=None, slopes=np.linspace(0.01, 1, 10), flows=[0.25, 0.5, 1, 2, 4]
):
    """
    plots the mean number of streams in a cross section dependent on slope for different water flow amounts
...
def analyze_num_streams_experiment_erosion_entropy(
    data_file,
    output_file=None,
    slopes=np.linspace(0.05, 1, 10),
    erosions=[0.025, 0.05, 0.1, 0.2, 0.4],
    ...
    mean_num_streams_graph_erosion_entropy(
        mean_entropies, output_file, slopes, erosions
    )


def mean_num_streams_graph_erosion_entropy(
    data,
    output_file=None,
    slopes=np.linspace(0.01, 1, 10),
    erosions=[0.025, 0.05, 0.1, 0.2, 0.4],
...
if __name__ == "__main__":
    ...
    analyze_num_streams_experiment_flow_entropy(
        "data/grids_data1.npy",
        "plots/slope_phase_trans_flow_entropy.png",
        slopes=slopes,
        flows=flows,
    )
    ...
    analyze_num_streams_experiment_erosion_entropy(
        "data/grids_data_erosion.npy",
        "plots/slope_phase_trans_erosion_entropy.png",
        slopes=slopes,
        erosions=erosions,
    )
